package mmr.littledelicacies.entity.misc;
...
public class MaidFishingBobberEntity extends Entity {
    private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager.createKey(MaidFishingBobberEntity.class, DataSerializers.VARINT);
    ...
    private MaidFishingBobberEntity.State currentState = MaidFishingBobberEntity.State.FLYING;
    ...
    @OnlyIn(Dist.CLIENT)
    public MaidFishingBobberEntity(World worldIn, LittleMaidEntity p_i47290_2_, double x, double y, double z) {
    ...
    public MaidFishingBobberEntity(EntityType<MaidFishingBobberEntity> maidFishingBobberEntityEntityType, World world) {
        super(maidFishingBobberEntityEntityType, world);
        this.angler = null;
        this.luck = 0;
        this.lureSpeed = 0;
    }

    public MaidFishingBobberEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
        this(LittleEntitys.MAID_FISHING_BOBBER, world);
    }
    ...
    public void tick() {
        ...
        if (this.angler == null) {
            ...
            if (this.currentState == MaidFishingBobberEntity.State.FLYING) {
                if (this.caughtEntity != null) {
                    this.setMotion(Vec3d.ZERO);
                    this.currentState = MaidFishingBobberEntity.State.HOOKED_IN_ENTITY;
                    return;
                }

                if (f > 0.0F) {
                    this.setMotion(this.getMotion().mul(0.3D, 0.2D, 0.3D));
                    this.currentState = MaidFishingBobberEntity.State.BOBBING;
                    return;
                }
                ...
                if (this.currentState == MaidFishingBobberEntity.State.HOOKED_IN_ENTITY) {
                    if (this.caughtEntity != null) {
                        if (this.caughtEntity.removed) {
                            this.caughtEntity = null;
                            this.currentState = MaidFishingBobberEntity.State.FLYING;
                        } else {
                ...
                if (this.currentState == MaidFishingBobberEntity.State.BOBBING) {
    ...
    public int handleHookRetraction(ItemStack p_146034_1_) {
        if (!this.world.isRemote && this.angler != null) {
            ...
            if (this.caughtEntity != null) {
                ...
                LootContext.Builder lootcontext$builder = (new LootContext.Builder((ServerWorld) this.world)).withParameter(LootParameters.POSITION, new BlockPos(this)).withParameter(LootParameters.TOOL, p_146034_1_).withRandom(this.rand).withLuck((float) this.luck);
