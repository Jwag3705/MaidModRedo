...
public class JobBookItem extends Item {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (context.getFace() != Direction.DOWN && world.getBlockState(blockpos.up()).isAir(world, blockpos.up())) {
            ...
            playerentity.sendStatusMessage(new TranslationTextComponent("item.littledelicacies.set_jobpostion", context.getPos().getX(), context.getPos().getY(), context.getPos().getZ()), true);
        ...
        if (target instanceof LittleMaidBaseEntity) {
            if (((LittleMaidBaseEntity) target).isTamed() && ((LittleMaidBaseEntity) target).isOwner(playerIn)) {
                ...
                if (compound != null && DimensionType.getById(compound.getInt("JobDimension")) != null && compound.contains("JobDimension") && compound.contains("JobPosX", 99) && compound.contains("JobPosY", 99) && compound.contains("JobPosZ", 99)) {
                    ...
                    playerIn.sendStatusMessage(new TranslationTextComponent("item.littledelicacies.setmaid_jobpostion", compound.getInt("JobPosX"), compound.getInt("JobPosY"), compound.getInt("JobPosZ")), true);
                }
