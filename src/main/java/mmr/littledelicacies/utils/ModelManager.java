...
import mmr.littledelicacies.api.classutil.FileClassUtil;
import mmr.littledelicacies.client.maidmodel.*;
...
import mmr.littledelicacies.entity.monstermaid.EnderMaidEntity;
import mmr.littledelicacies.entity.phantom.SugarPhantomEntity;
import mmr.littledelicacies.init.MaidModels;
...
public class ModelManager {
    ...
    public void init() {
        ...
        addSearch("LittleDelicacies", "/assets/littledelicacies/textures/entity/CowGirl/", "ModelCowGirl_");
        addSearch("LittleDelicacies", "/assets/littledelicacies/textures/entity/SqurrielMaid/", "ModelSqurrielMaid_");
        addSearch("LittleDelicacies", "/assets/littledelicacies/textures/entity/SqurrielButler/", "ModelSqurrielButler_");
        addSearch("LittleDelicacies", "/assets/littledelicacies/textures/entity/monstermaid/EnderMaid/", "EnderMaidModel_");
...
//		LittleMaidRedo.LOGGER.debug("getTextureBoxServer: %d / %d", pIndex, textureServer.size());
    ...
    public boolean loadTextures() {
        LittleDelicacies.LOGGER.debug("loadTexturePacks.");
        ...
        LittleDelicacies.LOGGER.debug("Rebuild Texture Lists.(%d)", textures.size());
        for (TextureBox lbox : textures) {

            if (lbox.getWildColorBits() > 0) {
                setTextureForType(lbox);
            }
            LittleDelicacies.LOGGER.debug("texture: %s(%s) - hasModel:%b", lbox.textureName, lbox.fileName, lbox.models != null);
        }
    ...
    private void searchFiles(File ln, String[] lst) {
        LittleDelicacies.LOGGER.debug("getTexture[%s:%s].", lst[0], lst[1]);
        // mods
        for (File lf : ln.listFiles()) {
            ...
            LittleDelicacies.LOGGER.debug("getTexture-append-%s-%s.", lf.getName(), lflag ? "done" : "fail");
        }
    }
...
*/
    ...
    public void initTextureList(boolean pFlag) {
        LittleDelicacies.LOGGER.debug("Clear TextureBoxServer.");
        ...
        if (pFlag) {
            ...
            LittleDelicacies.LOGGER.debug("Rebuild TextureBoxServer(%d).", textureServer.size());
        }
    }
...
//					cn = (new StringBuilder("")).append(".").append(cn).toString();
                ...
                modelMap.put(pn, mlm);
                LittleDelicacies.LOGGER.debug("getModelClass-" + pn + ":" + cn);
            } catch (Exception exception) {
                LittleDelicacies.LOGGER.debug("getModelClass-Exception: %s", fname);
                //if(DevMode.DEVELOPMENT_debug_MODE || LMRConfig.cfg_PrintdebugMessage) exception.printStackTrace();
            } catch (Error error) {
                LittleDelicacies.LOGGER.debug("getModelClass-Error: %s", fname);
...
//		LittleMaidRedo.LOGGER.debug("MMM_TextureManager.addTextureName : %s # %s # %s # %s", fname, pSearch[0], pSearch[1], pSearch[2]);
            ...
            if (pSearch[1].length() < i) {
                ...
                if (lindex > -1) {
                    ...
                    if (lts == null) {
                        ...
                        textures.add(lts);
                        LittleDelicacies.LOGGER.debug("getTextureName-append-texturePack-%s", pn);
                    }
                    //lts.addTexture(lindex, fname);
                    if (fname.contains("littledelicacies")) {
                        lts.addTexture(lindex, "littledelicacies:" + fname);
                    } else {
                        lts.addTexture(lindex, fname);
                    }
                    return true;
    ...
    protected boolean addTexturesZip(File file, String[] pSearch) {
        ...
        try {
            ...
            ZipEntry zipentry;
            LittleDelicacies.LOGGER.debug("Start searching %s", file.getName());
            do {
            ...
            LittleDelicacies.LOGGER.debug("addTextureZip-Exception.");
            return false;
        }
    }
...
//							addTextureName(s.substring(i).replace('\\', '/'));
            ...
            LittleDelicacies.LOGGER.debug("addTexturedebug-Exception.");
            return false;
        }
    }
...
//		int li = textureServerIndex.get(getDefaultTexture(pEntity));
    ...
    public void setDefaultTexture(Class pEntityClass, TextureBox pBox) {
        ...
        LittleDelicacies.LOGGER.debug("appendDefaultTexture:%s(%s)",
