...
public abstract class ManagerBase {
    ...
    private void startSearch(File root, boolean dev) {
        ...
        LittleDelicacies.LOGGER.debug("MC %s", mcv);
        LittleDelicacies.LOGGER.debug("START SEARCH MODS FOLDER");
        decodeDirectory(root, root);
        for (File lf : root.listFiles()) {
            if (lf.isFile() && (lf.getName().endsWith(".zip") || lf.getName().endsWith(".jar"))) {
                ...
                LittleDelicacies.LOGGER.debug("DIR SEARCH %s", md);
                String mf = FileClassUtil.getFileName(md);
                LittleDelicacies.LOGGER.debug(" SPLICE %s", mf);
                if (mf.equals(mcv)) {
                    LittleDelicacies.LOGGER.debug("DEBUG START SEARCH DIVIDED FOLDER");
    ...
    private void decodeZip(File pfile) {
        ...
        try {
            ...
            LittleDelicacies.LOGGER.debug("add%sZip-Exception.", getPreFix());
        }
...
// LMM_EntityModeManager でしか使ってないので暫定
                lclass = LittleDelicacies.class.getClassLoader().loadClass(lclassname);
            } else {
            ...
            if (append(lclass)) {
                LittleDelicacies.LOGGER.debug("get%sClass-done: %s", getPreFix(), lclassname);
            } else {
                LittleDelicacies.LOGGER.debug("get%sClass-fail: %s", getPreFix(), lclassname);
            }
			/*
