...
//		LittleMaidRedo.LOGGER.debug("getTextureBoxServer: %d / %d", pIndex, textureServer.size());
    ...
    public boolean loadTextures() {
        LittleMaidRedo.LOGGER.debug("loadTexturePacks.");
        ...
        LittleMaidRedo.LOGGER.debug("Loaded Texture Lists.(%d)", textures.size());
        for (TextureBox lbox : textures) {
            LittleMaidRedo.LOGGER.debug("texture: %s(%s) - hasModel:%b", lbox.textureName, lbox.fileName, lbox.models != null);
        }
        ...
        LittleMaidRedo.LOGGER.debug("Rebuild Texture Lists.(%d)", textures.size());
        for (TextureBox lbox : textures) {
    ...
    private void searchFiles(File ln, String[] lst) {
        LittleMaidRedo.LOGGER.debug("getTexture[%s:%s].", lst[0], lst[1]);
        // mods
        for (File lf : ln.listFiles()) {
            ...
            LittleMaidRedo.LOGGER.debug("getTexture-append-%s-%s.", lf.getName(), lflag ? "done" : "fail");
        }
    }
...
*/
    ...
    public void initTextureList(boolean pFlag) {
        LittleMaidRedo.LOGGER.debug("Clear TextureBoxServer.");
        ...
        if (pFlag) {
            ...
            LittleMaidRedo.LOGGER.debug("Rebuild TextureBoxServer(%d).", textureServer.size());
        }
    }
...
//					cn = (new StringBuilder("")).append(".").append(cn).toString();
                ...
                modelMap.put(pn, mlm);
                LittleMaidRedo.LOGGER.debug("getModelClass-%s:%s", pn, cn);
            }
            catch (Exception exception) {
                LittleMaidRedo.LOGGER.debug("getModelClass-Exception: %s", fname);
                //if(DevMode.DEVELOPMENT_debug_MODE || LMRConfig.cfg_PrintdebugMessage) exception.printStackTrace();
            }
            catch (Error error) {
                LittleMaidRedo.LOGGER.debug("getModelClass-Error: %s", fname);
...
//		LittleMaidRedo.LOGGER.debug("MMM_TextureManager.addTextureName : %s # %s # %s # %s", fname, pSearch[0], pSearch[1], pSearch[2]);
            ...
            if (pSearch[1].length() < i) {
                ...
                if (lindex > -1) {
                    ...
                    if (lts == null) {
                        ...
                        textures.add(lts);
                        LittleMaidRedo.LOGGER.debug("getTextureName-append-texturePack-%s", pn);
                    }
    ...
    protected boolean addTexturesZip(File file, String[] pSearch) {
        ...
        try {
            ...
            ZipEntry zipentry;
            LittleMaidRedo.LOGGER.debug("Start searching %s", file.getName());
            do {
            ...
            LittleMaidRedo.LOGGER.debug("addTextureZip-Exception.");
            return false;
        }
    }
...
//							addTextureName(s.substring(i).replace('\\', '/'));
            ...
            LittleMaidRedo.LOGGER.debug("addTexturedebug-Exception.");
            return false;
        }
    }
...
//		int li = textureServerIndex.get(getDefaultTexture(pEntity));
    ...
    public void setDefaultTexture(Class pEntityClass, TextureBox pBox) {
        ...
        LittleMaidRedo.LOGGER.debug("appendDefaultTexture:%s(%s)",
