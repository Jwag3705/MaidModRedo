...
public class HouseWandItem extends Item {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (context.getFace() != Direction.DOWN && world.getBlockState(blockpos.up()).isAir(world, blockpos.up()) && world.getBlockState(blockpos.up(2)).isAir(world, blockpos.up(2))) {
            ...
            playerentity.sendStatusMessage(new TranslationTextComponent("item.maidmodredo.setrest", context.getPos().getX(), context.getPos().getY(), context.getPos().getZ()), true);
        ...
        if (target instanceof LittleMaidBaseEntity) {
            if (((LittleMaidBaseEntity) target).isTamed() && ((LittleMaidBaseEntity) target).isOwner(playerIn)) {
                ...
                if (compound != null && compound.contains("RestDimension") && compound.contains("RestDimension") && compound.contains("RestPosX", 99) && compound.contains("RestPosY", 99) && compound.contains("RestPosZ", 99)) {
                    ...
                    playerIn.sendStatusMessage(new TranslationTextComponent("item.maidmodredo.setmaidrest", compound.getInt("RestPosX"), compound.getInt("RestPosY"), compound.getInt("RestPosZ")), true);
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if (compound != null && compound.contains("BedDimension")) {
            tooltip.add((new TranslationTextComponent("item.maidmodredo.housewand.bed", compound.getInt("BedPosX"), compound.getInt("BedPosY"), compound.getInt("BedPosZ"))));
        }
        if (compound != null && compound.contains("RestDimension")) {
            tooltip.add((new TranslationTextComponent("item.maidmodredo.housewand.rest", compound.getInt("RestPosX"), compound.getInt("RestPosY"), compound.getInt("RestPosZ"))));
        }
    }
}
