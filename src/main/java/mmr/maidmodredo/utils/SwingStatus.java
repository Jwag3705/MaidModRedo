...
public class SwingStatus {
     ...
     */
    public void onEntityUpdate(LittleMaidEntity pEntity) {
     ...
     */
    public void onUpdate(LittleMaidEntity pEntity) {
        prevSwingProgress = swingProgress;
        if (attackTime > 0) {
            attackTime--;
        }
        ...
        if (isUsingItem()) {
            ItemStack itemstack;
            try {
                itemstack = pEntity.getInventoryMaidMain().getStackInSlot(index);
            } catch (Exception e) {
                return;
            }
     ...
     */
    public ItemStack getItemStack(LittleMaidEntity pEntity) {
        if (index > -1) {
            return pEntity.getInventoryMaidMain().getStackInSlot(index);
        }
...
// アイテムの使用に関わる関数群
    ...
    public int getItemInUseDuration() {
        return isUsingItem() ? itemInUse.getUseDuration() - itemInUseCount : 0;
    }

    /**
     * @param pEntity サーバーの時はEntityを設定する。
     */
    public void stopUsingItem(Entity pEntity) {
        if (!itemInUse.isEmpty() && pEntity instanceof PlayerEntity) {
            itemInUse.onPlayerStoppedUsing(pEntity.getEntityWorld(), (PlayerEntity) pEntity, itemInUseCount);
        }
    ...
    public boolean isBlocking() {
        return isUsingItem() && this.itemInUse.getUseAction() == UseAction.BLOCK;
    }
    ...
    protected void updateItemUse(Entity pEntity, int par2) {
        if (itemInUse.getUseAction() == UseAction.DRINK) {
            pEntity.playSound(SoundEvents.ENTITY_GENERIC_DRINK, 0.5F, pEntity.getEntityWorld().rand.nextFloat() * 0.1F + 0.9F);
        }

        Random rand = new Random();

        if (itemInUse.getUseAction() == UseAction.EAT) {
            ...
            pEntity.playSound(SoundEvents.ENTITY_GENERIC_EAT, 0.5F + 0.5F * rand.nextInt(2), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
        }
    }

    protected void onItemUseFinish(PlayerEntity pPlayerEntity) {
        if (!this.itemInUse.isEmpty()) {
            this.updateItemUse(pPlayerEntity, 16);
            int var1 = this.itemInUse.getCount();

            // TODO いる？
            ItemStack var2 = itemInUse.onItemUseFinish(pPlayerEntity.getEntityWorld(), pPlayerEntity);

            if (var2 != this.itemInUse || !var2.isEmpty() && var2.getCount() != var1) {
                if (var2.getCount() == 0) {
                    pPlayerEntity.inventory.setInventorySlotContents(index, null);
                } else {
                    pPlayerEntity.inventory.setInventorySlotContents(index, var2);
                }
            }

            clearItemInUse(pPlayerEntity);
        }
    }
