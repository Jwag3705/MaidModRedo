package mmr.maidmodredo.utils.manager;
...
public abstract class ManagerBase {
    ...
    private void startSearch(File root, boolean dev) {
        ...
        MaidModRedo.LOGGER.debug("MC %s", mcv);
        MaidModRedo.LOGGER.debug("START SEARCH MODS FOLDER");
        decodeDirectory(root, root);
        for (File lf : root.listFiles()) {
            if (lf.isFile() && (lf.getName().endsWith(".zip") || lf.getName().endsWith(".jar"))) {
                ...
                MaidModRedo.LOGGER.debug("DIR SEARCH %s", md);
                String mf = FileClassUtil.getFileName(md);
                MaidModRedo.LOGGER.debug(" SPLICE %s", mf);
                if (mf.equals(mcv)) {
                    MaidModRedo.LOGGER.debug("DEBUG START SEARCH DIVIDED FOLDER");
    ...
    private void decodeZip(File pfile) {
        ...
        try {
            ...
            MaidModRedo.LOGGER.debug("add%sZip-Exception.", getPreFix());
        }
...
// LMM_EntityModeManager でしか使ってないので暫定
                lclass = MaidModRedo.class.getClassLoader().loadClass(lclassname);
            } else {
            ...
            if (append(lclass)) {
                MaidModRedo.LOGGER.debug("get%sClass-done: %s", getPreFix(), lclassname);
            } else {
                MaidModRedo.LOGGER.debug("get%sClass-fail: %s", getPreFix(), lclassname);
            }
			/*
